...
# Modified for Colab - use default parameters or command line args
parser = argparse.ArgumentParser(description="STTN")
parser.add_argument("-v", "--video", type=str, default="examples/schoolgirls_orig.mp4")
parser.add_argument("-m", "--mask",   type=str, default="examples/schoolgirls")
parser.add_argument("-c", "--ckpt",   type=str, default="checkpoints/sttn.pth")
parser.add_argument("--model",   type=str, default='sttn')

# For Colab usage - check if running in interactive mode
...
def run_video_inpainting(video_path=None, mask_path=None, ckpt_path=None):
    """
    Main function for video inpainting - can be called from Colab
    """
    ...
    if not os.path.exists(args.mask):
        print(f"Error: Mask directory {args.mask} not found!")
        return None
    ...
    output_filename = f"{os.path.splitext(os.path.basename(args.video))[0]}_result.mp4"
    ...
    writer.release()
    print('Finish in {}'.format(output_filename))
    return output_filename


def main_worker():
    return run_video_inpainting()
